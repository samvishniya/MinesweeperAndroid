...
public class Minefield {
    ...
    // recursively reveals 'safe' cells by calling revealCell()
    public void cascadeBlankCellReveal(int finalRowNum, int finalColumnNum) {
        ...
        for (int y = -1; y <= 1; y++) {
            for (int x = -1; x <= 1; x++) {
                ...
                if (x + finalRowNum >= 0 && x + finalRowNum <= dimension - 1 && y + finalColumnNum <= dimension - 1 && y + finalColumnNum >= 0) {
                    if (!fieldArray[x + finalRowNum][y + finalColumnNum].isHasMine() && !fieldArray[x + finalRowNum][y + finalColumnNum].isPressed()) {
                        revealCell(x + finalRowNum, y + finalColumnNum);
                    }
                }
...
// todo method to check if ALL cells already revealed -> gameWon =true/false ->
    public boolean revealCell(int finalRowNum, int finalColumnNum) {
        ...
        if (fieldArray[finalRowNum][finalColumnNum].getAdjacentMineCounter() == 0) {
